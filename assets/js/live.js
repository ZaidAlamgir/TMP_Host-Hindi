...
// });
        ...
        function extractLangContent(raw, lang = 'hi') {
            if (!raw) return '';
            const tagMap = {
                'hi': ['hindi', 'hi'],
                'ur': ['urdu', 'ur'],
            ...
            for (const t of tags) {
                const tagRegex = new RegExp(`(?:<${t}[^>]*>)([\\s\\S]*?)(?:<\\/${t}>|(?=<(?:urdu|ur|hindi|hi|english|en)|<!--|$))`, 'i');
        ...
        function hasHindiTranslation(postData) {
            ...
            const cleanHeadline = (hiHeadline || '').replace(/<[^>]*>/g, '').trim();
        ...
        function renderPost(postData, container, insertAtTop = false) {
            // STRICT RULE: If the post does not contain a Hindi translation, completely hide it
            if (!hasHindiTranslation(postData)) {
                return;
            }
            ...
            let pinnedBadgeHTML = postData.is_pinned ? `<span class="pinned-badge"><i class="fas fa-thumbtack fa-xs"></i><span class="ml-1.5">पिन किया गया</span></span>` : '';
            ...
            postElement.innerHTML = `<div class="live-post-content">
                ...
                <div class="post-footer">
                    ...
                    <button class="share-btn" data-post-id="${postData.id}" data-post-headline="${postHeadline || 'Live Update'}"><i class="fas fa-share-alt mr-2"></i>शेयर</button>
                </div>
            </div>`;
        ...
        async function fetchFullFeed(forceCacheBypass = false) {
             ...
             if (prefetchedData) {
                ...
                if (Array.isArray(parsed)) {
                    allPosts = parsed.filter(p => hasHindiTranslation(p));
             ...
             try {
                 ...
                 allPosts = data.filter(p => hasHindiTranslation(p));
                 return allPosts;
             } catch (error) { 
        ...
        async function loadMorePosts(isFullRefresh = false) {
             ...
             if (allPosts.length === 0 || isFullRefresh) {
                 ...
                 const hindiFeed = fullFeed.filter(p => hasHindiTranslation(p));
                 if (hindiFeed.length === 0) {
                    liveFeed.innerHTML = '';
                    loadMoreBtn.style.display = 'none'; 
                    noMorePostsMsg.textContent = "अभी कोई लाइव अपडेट उपलब्ध नहीं है।"; 
                    noMorePostsMsg.style.display = 'block';
                    return;
                 }
                 const pinned = hindiFeed.find(p => p.is_pinned);
                 if (pinned && (isFullRefresh || pinnedPostContainer.innerHTML.trim() === '')) {
                     pinnedPostContainer.innerHTML = '';
                     renderPost(pinned, pinnedPostContainer, false);
                 }
                 allPosts = hindiFeed.filter(p => !p.is_pinned); 
             }
            ...
            if (loadedPostsCount >= allPosts.length) {
                ...
                loadMoreBtn.textContent = 'पिछले अपडेट्स लोड करें'; 
        ...
        if (totalPostsOnScreen > 0) {
            ...
            if (cachedData) { 
                ...
                const loadMoreBtn = document.getElementById('load-more-btn');
                allPosts = fullFeed.filter(p => !p.is_pinned && hasHindiTranslation(p)); 
                ...
                if (allPosts.length > 0 && totalPostsOnScreen >= allPosts.length) {
                     ...
                     if(loadMoreBtn) { loadMoreBtn.style.display = 'inline-block'; loadMoreBtn.disabled = false; loadMoreBtn.textContent = 'पिछले अपडेट्स लोड करें'; }
                }
            } else {
                fetchFullFeed(false).then((data) => {
                     const loadMoreBtn = document.getElementById('load-more-btn');
                     const hindiPosts = data.filter(p => hasHindiTranslation(p));
                     const feedCount = hindiPosts.length - (hindiPosts.find(p=>p.is_pinned)?1:0);
